...
$worker->onMessage = function($connection, $data)
{
    ...
    // 判断数据是否正确
    if(empty($data['server']) || empty($data['class']) || empty($data['method']) || !isset($data['param_array']))
    {
        // 发送数据给客户端，请求包错误
    ...
    if(!class_exists($class))
    {
        ...
        //use Json\Service;
        $class  = "$server_name\\Service\\" . $class;
        var_dump($class);
    ...
    try 
    {
        ...
        return $connection->send(array('code'=>0, 'msg'=>'success', 'data'=>$ret));
    }
    // 有异常
    catch(Exception $e)
    {
